...
test.describe('Proposal Review UI', () => {
  ...
  test('should display proposal review section when proposals exist', async ({ page }) => {
    // 提案セクションが表示されることを確認
  ...
  test('should display new task proposal details correctly', async ({ page }) => {
  ...
  test('should display update task proposal details correctly', async ({ page }) => {
  ...
  test('should toggle proposal section visibility', async ({ page }) => {
  ...
  test('should handle approve action', async ({ page }) => {
    // モックレスポンスを設定
    await page.route('/api/backlog/proposal/approve', async (route) => {
    ...
    // 提案リストの更新をモック
    await page.route('/api/backlog/proposal', async (route) => {
  ...
  test('should handle reject action', async ({ page }) => {
    // モックレスポンスを設定
    await page.route('/api/backlog/proposal/reject', async (route) => {
      const requestData = JSON.parse(route.request().postData() || '{}');
      await route.fulfill({
        status: 200,
        body: JSON.stringify({
  ...
  test('should handle modify action', async ({ page }) => {
    // 修正ボタンをクリック
    ...
    // モックレスポンスを設定
    await page.route('/api/backlog/proposal/modify', async (route) => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify({
          success: true,

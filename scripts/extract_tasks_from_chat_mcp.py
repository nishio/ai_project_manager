"""
非構造化メモからタスクを抽出するスクリプト (MCP版)
Script to extract tasks from unstructured memos using MCP
...
5. MCP (Model Context Protocol) を使用 / Uses MCP

Usage:
    python extract_tasks_from_chat_mcp.py [input_file]

Example:
    python extract_tasks_from_chat_mcp.py tmp_chatlog.txt
"""
...
def generate_task_extraction_prompt(segment: Dict[str, str]) -> str:
    """
...
async def extract_tasks_from_segment(segment: Dict[str, str]) -> List[Dict]:
    """
    Extract tasks from a single chat segment using MCP.
    ...
    """
    system_prompt = generate_task_extraction_prompt(segment)
    messages = [
    ...
    response = await client.call_with_tools(messages)
...
def load_progress() -> tuple:
    ...
    progress_file = "extract_tasks_progress_mcp.json"
...
async def process_segments(segments, start_segment, unassigned_patches, total_segments):
    ...
    for i, segment in enumerate(segments[start_segment:], start_segment + 1):
        ...
        for segment_attempt in range(max_segment_retries):
            try:
                ...
                patches = await extract_tasks_from_segment(segment)
                if patches:
...
async def main_async():
    ...
    try:
        ...
        unassigned_patches = await process_segments(segments, start_segment, unassigned_patches, total_segments)

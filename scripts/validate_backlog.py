    ...
    try:
        with open(filepath, "r", encoding="utf-8") as f:
            ...
            for task in tasks:
                ...
                if missing:
                    print(
                        f"Task {task.get('id', 'UNKNOWN')} missing required fields: {missing}"
                    )
                    return False
                ...
                if not is_valid_temp_id(task["id"]):
                    print(
                        f"Task {task['id']}: invalid temporary ID format (must be TXXXX)"
                    )
                    return False
                ...
                if "permanent_id" in task:
                    if not is_valid_permanent_id(task["permanent_id"]):
                        print(
                            f"Task {task['id']}: invalid permanent ID format (must be UUID)"
                        )
                        return False
                ...
                if "labels" in task and not isinstance(task["labels"], list):
                    ...
                    return False
                if "assignable_to" in task and not isinstance(
                    task["assignable_to"], list
                ):
                ...
                if "dependencies" in task:
                    ...
                    for dep_type in ["must", "nice_to_have", "human"]:
                        if dep_type in deps and not isinstance(deps[dep_type], list):
                            print(
                                f"Task {task['id']}: dependencies.{dep_type} must be list"
                            )
                            return False

            print("backlog.json exists and follows required structure")
            return True

    except FileNotFoundError:
        print("backlog.json not found")
        return False
